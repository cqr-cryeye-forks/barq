...
def shellscript_options(system_name):
    """
    ...
    if payload == AttackType.REVERSE_SHELL.value or payload == AttackType.MSF.value:
        print_color(
        ...
        if payload == AttackType.REVERSE_SHELL.value:
            payload, action = reverseshell_options(remote_ip_host, remote_port, system_name)
        elif payload == AttackType.MSF.value:
            payload, action = metasploit_installed_options(remote_ip_host, remote_port, system_name)
        disable_av = True
    elif payload == AttackType.URL.value:
...
def reverseshell_options(host: str, port: str, system_name: str) -> (str, str):
    """
